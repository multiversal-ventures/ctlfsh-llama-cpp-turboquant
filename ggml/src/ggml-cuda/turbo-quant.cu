...
// ============================================================================
// turbo_split2 set_rows kernel — one thread per 128-element block
// ============================================================================
template <typename idx_t>
static __global__ void k_set_rows_turbo_split2(
        const float * __restrict__ src0,
        const idx_t * __restrict__ src1,
    ...
    const int64_t blocks_per_row = ne00 / QK_TURBO_SPLIT2;
    const int64_t total_blocks = blocks_per_row * ne01 * ne02 * ne03;
    const int64_t i = (int64_t)blockDim.x * blockIdx.x + threadIdx.x;
    ...
    const float * src_blk = src0 + i01*s01 + i02*s02 + i03*s03 + i_blk * QK_TURBO_SPLIT2;

    block_turbo_split2_0 * dst_row_ptr = (block_turbo_split2_0 *)((char *)dst + dst_row*nb1 + i02*nb2 + i03*nb3);
    block_turbo_split2_0 * dst_blk = dst_row_ptr + i_blk;

    quantize_f32_turbo_split2_0_block(src_blk, dst_blk);
...
// ============================================================================
// Host dispatch for turbo_split2 set_rows
// ============================================================================
template <typename idx_t>
static void set_rows_cuda_turbo_split2(
        const float * src0_d, const idx_t * src1_d, block_turbo_split2_0 * dst_d,
        const int64_t ne00, const int64_t ne01, const int64_t ne02, const int64_t ne03,
        const int64_t ne10, const int64_t ne11, const int64_t ne12,
        const size_t nb01, const size_t nb02, const size_t nb03,
        const size_t nb10, const size_t nb11, const size_t nb12,
    ...
    GGML_ASSERT(ne00 % QK_TURBO_SPLIT2 == 0);
    ...
    k_set_rows_turbo_split2<<<grid_size, block_size, 0, stream>>>(
        src0_d, src1_d, dst_d,
        ne00, ne01, ne02, ne03,
        ne10, ne11, ne12,
        nb01/sizeof(float), nb02/sizeof(float), nb03/sizeof(float),
...
void ggml_cuda_set_rows_turbo_split2(ggml_backend_cuda_context & ctx, const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst) {
    ...
    set_rows_cuda_turbo_split2(
        src0_d, src1_d, (block_turbo_split2_0 *)dst->data,
        ne00, ne01, ne02, ne03,
        ne10, ne11, ne12,
        nb01, nb02, nb03,
        nb10, nb11, nb12,
...
template void ggml_cuda_set_rows_turbo_split2<int32_t>(ggml_backend_cuda_context &, const ggml_tensor *, const ggml_tensor *, ggml_tensor *);
template void ggml_cuda_set_rows_turbo_split2<int64_t>(ggml_backend_cuda_context &, const ggml_tensor *, const ggml_tensor *, ggml_tensor *);
